...
function App() {
  ...
  const [targetUrl, setTargetUrl] = useState('https://www.buysmart.ai/chat/Cv3UeFfyRE2h6xTsOjqnx');
  ...
  const handleStartCall = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (DailyCall) {
      setLoading(true);
      try {
        const conversation = await createConversation(targetUrl);
...
//https://www.buysmart.ai/chat/Cv3UeFfyRE2h6xTsOjqnx
      ...
      ><input type="text" value={targetUrl} onChange={(e)=> setTargetUrl(e.target.value)}/> 
       <form onSubmit={handleStartCall} className="token-form">
          <button 
            ...
            type="submit"
            className="btn btn-primary w-48 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
          >
